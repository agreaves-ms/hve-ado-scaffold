...
When summarizing:
* Must always keep all inputs and their values provided by prompt in summary
* Must retain derived values (such as prId, etc) in summary
* Must provide full paths to all files being edited or read in (this instruction file, the prompt that kicked off the process)

After summarizing:
* Must read in and follow all of these instructions
* Must continue to collect information and write out relevant information to file
